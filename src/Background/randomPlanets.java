...
public class randomPlanets {
    ...
    private final int anzahlRandomPlaneten = 220;

    public randomPlanets(AssetManager assetManager, Node rootNode) {
    ...
    float minDistance = 110.0f; // Der minimale Abstand zwischen den Objekten
    ...
    for (int i = 0; i < anzahlRandomPlaneten; i++) {
        ...
        */        
        ...
        while (!validPosition) {
            float x = random.nextFloat() * 1300 - 500; // Range: -500 to 700
            float y = random.nextFloat() * 1800 - 500; // Range: -500 to 1300
            float z = random.nextFloat() * 1400 - 700; // Range: -700 to 700
            ...
            if (validPosition) {
                ...
                // Random scale
                float scaleFactor = random.nextFloat() * 3; //Range 0 to 3 
                
                randomObjModel.scale(scaleFactor);

                Material mat = new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
                mat.setTexture("ColorMap", assetManager.loadTexture("Texture/test.png")); // Change the path to your texture
            
                Material mat2 = new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
                mat2.setTexture("ColorMap", assetManager.loadTexture("Texture/textureMars.png")); // Change the path to your texture

                //zufaellige Zuordnung von Material
                if (random.nextFloat() < 0.4f) {
                    randomObjModel.setMaterial(mat);
                } else {
                    randomObjModel.setMaterial(mat2);
                }
